...
| `watchCollection.json` | `Movie` / `TVSeries` | Imported from Trakt export CSVs. `imdb_id`→`imdbId` (kept flat) + `sameAs` (IMDb URL); title/year/poster enriched via TMDB `/find`; `rating`→`review.reviewRating`; `watched_at`/`watchlisted_at`→`addedAt`/`watchlistedAt` |
| `listenCollection.json` | `MusicAlbum` | `artist`→`byArtist` (MusicGroup); title/year/cover enriched via MusicBrainz release-group search + Cover Art Archive; `musicBrainzId` kept flat; `rating`→`review.reviewRating` |
...
**`musicCollection.json`** — 314 records (171 CD, 143 vinyl), static archive. Reference standard is MusicBrainz, not Discogs (open, nonprofit, CC0/PDDL vs. Discogs' auth-gated API). `id` is array order = physical shelf order — meaningful, never reorder. Covers migrated off base64 (`vinyl_covers.json`/`cd_covers.json`, now deleted) to `src/assets/img/music-covers/{id}.jpg` (312/314; 2 unresolved). `genres`: consolidated from 154 raw words into 23 fixed buckets (self-curated, not folksonomy). `barcode` present on 177/314 — usable as a future cross-collection bridge (see `docs/features.md`). Schema.org field rename (`artist`→`byArtist`, `catalogue_number`→`catalogNumber`, `genres`→`genre`, `label`→`recordLabel`, `@type: MusicRelease`) was documented ahead of the actual data change for a while — applied for real when `listenCollection.json` was built, so the two collections would be consistent.

**`listenCollection.json`** — 7 albums, an active "in rotation" listening log (schema.org `MusicAlbum` — the abstract work, not a specific pressing, since there's no release-specific data like `musicCollection.json`'s catalogue numbers). Enriched via MusicBrainz release-group search + Cover Art Archive (`/front-500` thumbnail, not full-res — first pass pulled multi-MB originals, re-fetched smaller); covers cached to `src/assets/img/listen-covers/`. 7/7 matched. All entries currently unrated (`review: null`) — nothing's been rated/dated yet, consistent with the source being an active queue rather than a completed log.
...
**`watchCollection.json`** — 148 watched (movies/TV) + 18 watchlist, from a Trakt.tv export (`trakt_watched.csv`/`trakt_watchlist.csv`, since deleted — a re-import to Trakt stays possible later since `imdbId`/`review.datePublished`/`review.reviewRating.ratingValue` preserve the same granularity Trakt's own sync API takes). Title/year/poster enriched via TMDB's `/find` (imdb_id lookup); posters cached to `src/assets/img/watch-posters/`. 1/166 entries (`tt33810365`) has no TMDB match — left as a documented gap, same pattern as book-008's missing OL edition data. **Open question:** long-term home/utility on the site undecided, and whether Trakt-as-source-of-truth is the right long-term storage model — both deferred.
